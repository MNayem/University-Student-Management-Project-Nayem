<?php
	//header('location:project.php');
?>

<html>
	<head>
		<link href="project.css" rel="stylesheet" type="text/css"></link>
			<script src="project.js" type="text/javascript"></script>
		<title>all undergraduate program of NWU</title>
	</head>
	<body onload="startTime()">
		<div id="txt"></div>
		<div class="main">
			<div class="banner">
					<div class="banner_text">
						<h2>Computer Science and Engineering(CSE)</h2>
						<h6>Department of North Western University,Khulna</h6>
					</div>
					<a href="undergraduate.php"><img src="images/NWU.jpg" alt=""></img></a>
			</div>
			<div class="mainmenu">
				<ul class="menu">
					<ul>
						<li class="submenu"><a href="">About</a>
							<ul class="hidden">
								<li><a href="about.php" target="">About us</a></li>
								<li><a href="Nwu_trust.php" target="">NWU trust organogram</a></li>
								<li><a href="mission.php" target="">Mission</a></li>
								<li><a href="vision.php" target="">Vision</a></li>
								<li><a href="carrer.php" target="">Career of @CSE</a></li>
							</ul>
						</li>
						<li class="submenu"><a href="">Academic</a>
							<ul class="hidden">
								<li><a href="dean.php" target="">Dean</a></li>
								<li><a href="register.php" target="">Register</a></li>
								<li><a href="dept_head.php" target="">Head Of Department</a></li>
							</ul>
						</li>
						<li class="submenu"><a href="">Admission</a>
							<ul class="hidden">
								<li><a href="admission_notice.php" target="">Admission Notice</a></li>
								<li><a href="admission_notice.php" target="">Admission Form</a></li>
								<li><a href="graduate.php" target="">Graduate</a></li>
								<li><a href="undergraduate.php" target="">Under Graduate</a></li>
								<li><a href="scholarship_waiver.php" target="">Scholarship & Waiver</a></li>
							</ul>
						</li>
						<li class="submenu"><a href="">Courses</a>
							<ul class="hidden">
							<li><a href="first_year.php" target="">1st Year</a></li>
								<li><a href="second_year.php" target="">2nd Year</a></li>
								<li><a href="Third_year.php" target="">3rd Year</a></li>
								<li><a href="fourth_year.php" target="">4th Year</a></li>
							</ul>
						</li>
						<li class="submenu"><a href="">Administration</a>
							<ul class="hidden">
								<li><a href="dean_office.php" target="">Office of Dean</a></li>
								<li><a href="dept_head_office.php" target="">Office of Department Head</a></li>
								<li><a href="reception.php" target="">Reception</a></li>
								<li><a href="library_office.php" target="">Library Office</a></li>
							</ul>
						</li>
						<li><a href="library.php">Library</a></li>
						<li><a href="research.php">Research</a></li>
						<li><a href="ptoto_gallery.php">Photo Gallery</a></li>
					</ul>
				</ul>
			</div>
			<div class="about">
				<div class="left_side">
					<ul>
						<li><a href="facility.php">Facilities</a></li>
						<li><a href="lab.php">Lab</a></li>
						<li><a href="fees.php">Fees</a></li>
						<li><a href="mission.php">Mission</a></li>
						<li><a href="objective.php">Objectives</a></li>
						<li><a href="management.php">Management</a></li>
						<li><a href="scholarship_waiver.php">Tuition Weaver</a></li>
						<li><a href="scholarship_waiver.php">Scholarship</a></li>
						<li><a href="admission_notice.php">Download Form</a></li>
						<li><a href="contact.php">Contact us</a></li>
						<li><a href="project.php">Home</a></li>
					</ul>
				</div>
				<div class="right_side">
					<h2>All Undergraduate Program at NWU with approximate total cost :</h2>
					<table border="5" bgcolor="white" align="center" width="685">
						<tr>
							<td colspan="20" align="center" bgcolor="sky-blue">
								<h1>All undergraduate Program with Cost</h1>
							</td>
						</tr>
						<tr align="center">
							<th>Name of faculties :</th>
							<th>Name of programs :</th>
							<th>Duration :</th>
							<th>Credit :</th>
							<th>Total cost :</th>
						</tr>
						<tr align="center">
							<td>Science & Technology</td>
							<td>BSC in CSE</td>
							<td>4 Years</td>
							<td>162.00</td>
							<td>Tk.3,50,000/=</td>
						</tr>
						<br>
						<tr align="center">
							<td>Science & Technology</td>
							<td>BSC in CSE</td>
							<td>3 Years</td>
							<td>142.75</td>
							<td>Tk.2,96,764/=</td>
						</tr>
						<tr align="center">
							<td>Science & Technology</td>
							<td>BSC in EEE</td>
							<td>4 Years</td>
							<td>162.75</td>
							<td>Tk.4,00000/=</td>
						</tr>	
						<tr align="center">
							<td>Science & Technology</td>
							<td>BSC in EEE</td>
							<td>3 Years</td>
							<td>141.75</td>
							<td>Tk.3,33,790/=</td>
						</tr>	
						<tr align="center">
							<td>Science & Technology</td>
							<td>BSC in ECE</td>
							<td>4 Years</td>
							<td>162.00</td>
							<td>Tk.3,75,000/=</td>
						</tr>
						<tr align="center">
							<td>Science & Technology</td>
							<td>BSC in ECE</td>
							<td>3 Years</td>
							<td>140.25</td>
							<td>Tk.3,13,000/=</td>
						</tr>	
						<tr align="center">
							<td>Science & Technology</td>
							<td>BSC in Civil Engineering</td>
							<td>4 Years</td>
							<td>162.00</td>
							<td>Tk.4,40,000/=</td>
						</tr>	
						<tr align="center">
							<td>Science & Technology</td>
							<td>BSC in Civil Engineering</td>
							<td>3 Years</td>
							<td>140.00</td>
							<td>Tk.3,70,000/=</td>
						</tr>	
						<tr align="center">
							<td>Liberal Arts and Human Science</td>
							<td>LLB(Hons)</td>
							<td>4 Years</td>
							<td>140.00</td>
							<td>Tk.2,20,000/=</td>
						</tr>	
						<tr align="center">
							<td>Liberal Arts and Human Science</td>
							<td>BA(Hons) in English</td>
							<td>4 Years</td>
							<td>136.00</td>
							<td>Tk.2,30,000/=</td>
						</tr>	
						<tr align="center">
							<td>Liberal Arts and Human Science</td>
							<td>BSS(Hons) in Economics</td>
							<td>4 Years</td>
							<td>167.00</td>
							<td>Tk.2,20,000/=</td>
						</tr>	
						<tr align="center">
							<td>Business Administration</td>
							<td>BBA</td>
							<td>4 Years</td>
							<td>129.00</td>
							<td>Tk.3,10,000/=</td>
						</tr>	
					</table>
					<br>
					<h2>Proposed Undergraduate Program with approximate total cost :</h2>
					<table border="5" bgcolor="white" align="center" width="685">
					<br>
						<tr>
							<td colspan="20" align="center" bgcolor="sky-blue">
								<h1>Proposed Undergraduate Program</h1>
							</td>
						</tr>
						<tr align="center">
							<th>Name of faculties :</th>
							<th>Name of programs :</th>
							<th>Duration :</th>
							<th>Credit :</th>
							<th>Total cost :</th>
						</tr>
						<tr align="center">
							<td>Science & Technology</td>
							<td>BSC in Architecture</td>
							<td>5 years</td>
							<td>194-230</td>
							<td>Tk.6,50,000/=</td>
						</tr>
						<tr align="center">
							<td>Liberal Arts & Human Science</td>
							<td>BSS(Hons) in Sociology</td>
							<td>4 Years</td>
							<td>144.00</td>
							<td>Tk.1,80,000/=</td>
						</tr>
					</table>
				</div>
			</div>
		</div>
		<div class="footer">
				<a href="http://www.facebook.com"><img src="images/facebook-logo.png" height="25px" width="25px"></img></a>
				<p>@copy; CSE discipline of NWU All Rights Reserved</p>
        </div>
	</body>
</html>